...
class PowerFCResponseParser {
    ...
    parseResponse(command, response) {
        ...
        switch (command.hex) {
            ...
            case PowerFCCommands.basic.hex:
                this.parseBasic(response);
                break;
            case PowerFCCommands.aux.hex:
                this.parseAux(response);
                break;
            default:
    ...
    parseBasic(response) {
        ...
        if (typeof this.dataCallback == "function") {
            ...
            console.warn("parseBasic this.dataCallback is undefined");
        }
    }
    ...
    parseAdvanced(response) {
        ...
        if (typeof this.dataCallback == "function") {
            ...
            console.warn("parseAdvanced this.dataCallback is undefined");
        }
    }
    ...
    parseSensorData(response) {
        ...
        let booleanSensorFlags = BitArray.fromBuffer(response.slice(18, 20)).__bits;
        ...
        if (typeof this.dataCallback == "function") {
            ...
            console.warn("parseSensorData this.dataCallback is undefined");
        }
    }

    parseAux(response) {
        if (!response || response.length != PowerFCCommands.aux.expectedBytes) { 
            console.warn("PowerFCResponseRouter parseAux called with incorrect byte length of " + response.length + ": ", response);
            return; 
        }

        const data = {
            command: PowerFCCommands.aux,
            an1: response.slice(2, 3),
            an2: response.slice(3, 4),
            an3: response.slice(4, 5),
            an4: response.slice(5, 6)
        };

        if (typeof this.dataCallback == "function") {
            this.dataCallback(data);
        } else {
            console.warn("parseAux this.dataCallback is undefined");
        }
    }
...
const parser = new PowerFCResponseParser();
